...
import { ArrowLeft, Plus, ShoppingBag, Pencil, Trash2, Loader2 } from 'lucide-react';
...
const ShopkeeperProducts = () => {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      // Check if we should open add form
  ...
  const resetForm = () => {
    ...
    setFormDiscount('0'); setFormInStock(true); setFormImageFile(null); setFormImagePreview('');
          ...
          <div key={product.id} className="bg-card rounded-xl border border-border p-3 relative">
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <div className="flex gap-1">
                <button onClick={() => openEditForm(product)} className="p-1.5 text-muted-foreground hover:text-primary">
                  <Pencil className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => setDeleteId(product.id)} className="p-1.5 text-muted-foreground hover:text-destructive">
        ...
        <SheetContent side="bottom" className="max-h-[90vh] overflow-y-auto rounded-t-2xl">
          ...
          <div className="space-y-4 mt-4 pb-6">
            ...
            <div>
              ...
              <Input value={formName} onChange={e => setFormName(e.target.value)} className="h-12 rounded-xl" placeholder="e.g., Basmati Rice 5kg" />
            </div>
            <div>
              <label className="text-sm font-medium mb-1.5 block">Description</label>
              <Input value={formDesc} onChange={e => setFormDesc(e.target.value)} className="h-12 rounded-xl" placeholder="Brief description" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input type="number" value={formPrice} onChange={e => setFormPrice(e.target.value)} className="h-12 rounded-xl" placeholder="0" />
              </div>
              <div>
                <label className="text-sm font-medium mb-1.5 block">Discount %</label>
                <Input type="number" value={formDiscount} onChange={e => setFormDiscount(e.target.value)} className="h-12 rounded-xl" placeholder="0" />
              </div>
            </div>
